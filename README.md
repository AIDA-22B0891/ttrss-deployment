...
1) Написан модуль `R/ttrss_client.R` с использованием библиотеки `httr2`
2) Реализована авторизация через API и получение `session_id`
3) Настроена фильтрация и получение только непрочитанных (fresh) новостей
4) Данные автоматически конвертируются из JSON в R DataFrame

## ✅ 2.2. Интеграция с YandexGPT (LLM)
Написан модуль R/llm_client.R для взаимодействия с Yandex Cloud Foundation Models

Настроен системный промпт для выполнения трех задач за один проход:

Классификация (тегирование новости, макс. 2 слова)

Суммаризация (создание тизера, макс. 1 предложение)

Реализован парсинг ответа нейросети из JSON в структуру списка R

## ✅ 2.3. Сборка ETL-пайплайна
Создан управляющий скрипт R/pipeline.R, объединяющий получение данных и анализ

Настроена работа с переменными окружения (.env) для безопасности ключей API

Реализован механизм сохранения обработанных данных (Database)

Проведено успешное тестирование на реальных новостях
